...
public class SchoolMenus {
    ...
    public UssdResponse processRequest(SubscriberInfo sub, UssdRequest req) {
        ...
        switch (sub.getMenuLevel()) {
            case 2:
                return handleEnterID(sub, req);
            case 3:
                return handleEnterAmount(sub, req);
            case 4:
                return handleTransactionRequest(sub, req);
            case 5:
    ...
    private UssdResponse handleEnterID(SubscriberInfo sub, UssdRequest request) {
        Logger.getLogger("qos_ussd_processor").info("processEnterIDSchoolMenu for " + request.getMsisdn());
        final UssdResponse resp = new UssdResponse();
...
//            }
        ...
        final String respMessage = "Please enter your Student ID and Comment:";//UssdConstants.MESSAGES.getProperty(USSDSessionHandler.MessageKey.TVM_MAIN_MENU.toString());
        resp.setApplicationResponse(respMessage);
    ...
    private UssdResponse handleEnterAmount(SubscriberInfo sub, UssdRequest request) {
        Logger.getLogger("qos_ussd_processor").info("processEnterAmountSchoolMenu for " + request.getMsisdn());
        final UssdResponse resp = new UssdResponse();
        resp.setMsisdn(request.getMsisdn());
        try {
            sub.getSubParams().put("STUDENT_INFO", request.getSubscriberInput());
        } catch (NumberFormatException ex) {
        ...
        final String respMessage = "Please enter amount to Pay:";//UssdConstants.MESSAGES.getProperty(USSDSessionHandler.MessageKey.TVM_MAIN_MENU.toString());
        resp.setApplicationResponse(respMessage);
        resp.setFreeflow(UssdConstants.CONTINUE);
        sub.incrementMenuLevel();
        activeSessions.put(sub.getMsisdn(), sub);
    ...
    private UssdResponse handleTransactionRequest(SubscriberInfo sub, UssdRequest request) {
        Logger.getLogger("qos_ussd_processor").info("processTransactionRequestSchoolMenu for " + request.getMsisdn());
        final UssdResponse resp = new UssdResponse();
        resp.setMsisdn(request.getMsisdn());
        try {
    ...
    private UssdResponse handleConfirmTransaction(SubscriberInfo sub, UssdRequest request) {
        Logger.getLogger("qos_ussd_processor").info("processConfirmTransactionMenu for " + request.getMsisdn());
        final UssdResponse resp = new UssdResponse();
        resp.setMsisdn(request.getMsisdn());
    ...
    private String processPayment(SubscriberInfo sub, String sessionid) {
        ...
        Logger.getLogger("qos_ussd_processor").info("processing School transaction " + sub.getMsisdn());
    ...
    UssdResponse showMainMenu(SubscriberInfo sub) {
        ...
        final String respMessage = "School Name\n\n1. School Fee \n2. Text Books\n3. Others";
        resp.setApplicationResponse(respMessage);
        resp.setFreeflow(UssdConstants.CONTINUE);
        sub.incrementMenuLevel();
        //sub.setMerchantName();
